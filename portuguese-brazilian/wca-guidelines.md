...
- 1h1a+) [ESCLARECIMENTO] Juízes que estiverem competindo em uma tentativa de 3x3x3 em Menos Movimentos que estão julgando não devem avaliar tentativas até terem terminado a sua tentativa. Um juiz pode começar a avaliar tentativas antes do fim dos 60 minutos se já tiver submetido sua própria tentativa (ver Orientação E2b+)[Guidelines:guideline:E2b+]. É recomendado que juizes não avaliem suas próprias tentativas.
...
- 2c+) [ADIÇÃO] Novos competidores devem se registrar utilizando o seu nome legal ou o nome pelo qual preferem ser chamados na vida real. Sobrenomes nomes deve sem sobrenomes nomes legais.
...
- 2d++) [LEMBRETE] Se um terceiro (p. ex. jornalista) pedir à equipe de organização para ser colocado em contato com qual(is)quer competidor(es), primeiro deve ser obtido o consentimento do(s) competidor(es).
...
- 2i+) [ADIÇÃO] Embora o competidor possa pegar o cronômetro para ver o tempo atual (quando não estiver vendado), ele não deve começar, parar, pausar ou, de qualquer outra foram, interagir com a cronometragem da tentativa. Exceção: O competidor pode parar o cronômetro ao fim da sua tentativa se o juíz não estiver imediatamente disponível (ver Regra A6a1)[regulations:regulation:A6a1).
- 2i++) [ADIÇÃO] A equipe de organização pode disponibilizar ao competidor um cronômetro não oficial para ver o tempo decorrido (iniciado juntamente com o cronômetro principal). Neste caso, não é permitido que o competidor toque no cronômetro oficial. Exceção: Ver a exceção na Orientação 2i+[guidelines:guideline:2i+].
- 2i+++) [LEMBRETE] Quebra-cabeças bluetooth são considerados aparelhos eletrônicos.
- 2i++++) [ESCLARECIMENTO] O competidor é considerado usando um aparelho eletrônico somente se o estiver empregando a um propósito específico.
- 2i+++++) [EXEMPLO] Exemplos que não são considerados usos de um aparelho eletrônico: mover uma câmera, virar um celular de cabeça para baixo, vestir um smartwatch.
- 2i++++++) [EXEMPLO] Exemplos de uso de aparelho eletrônico: apertar um botão de câmera, checar uma mensagem em um celular ou smartwatch.
